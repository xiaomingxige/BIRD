...
cocoGt_path         = './DAUB_instances_test2017.json'
...
class MAP_vid(object):
    ...
    #   检测图片
    #---------------------------------------------------#
    def detect_image(self, images_path_list, record, images, results):
        ...
        with torch.no_grad():
            ...
            for i in range(num_frame):
                ...
                if image_path not in record:
                    record.append(image_path)
                    image_id = len(record)

                    ####################
                    for j, c in enumerate(top_label):
                        result                      = {}
                        top, left, bottom, right    = top_boxes[j]

                        result["image_id"]      = image_id
                        result["category_id"]   = clsid2catid[c]
                        result["bbox"]          = [float(left), float(top), float(right-left), float(bottom-top)]
                        result["score"]         = float(top_conf[j])
...
if __name__ == "__main__":
    ...
    if map_mode == 0 or map_mode == 1:
        ...
        with open(os.path.join(temp_save_path, 'eval_results.json'), "w") as f:
            results = []
            results, record = yolo.detect_image(images_path_list, record, images_list, results)
    ##########################################################
