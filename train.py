...
if __name__ == "__main__":
    ...
    CUDA_VISIBLE_DEVICES=1 nohup python -u train.py > nohup_DAUB.out &

    CUDA_VISIBLE_DEVICES=1 nohup python -u train.py > nohup_IRDST.out &
    """
    DATA_PATH = "./DAUB/images/"
    ...
    #------------------------------------------------------#
    num_fram = 5
    model = Network(num_classes=1, num_frame=num_fram)
    weights_init(model)
    if model_path != '':
        if local_rank == 0:
    ...
    train_dataset = seqDataset(train_lines, input_shape[0], num_fram, 'train')
    val_dataset = seqDataset(val_lines, input_shape[0], num_fram, 'val')
    if distributed:
    ...
    num_train   = train_dataset.length
    num_val     = val_dataset.length
    ...
    if True:
        ...
        #   冻结一定部分训练
        #------------------------------------#
        # for param in model.fea_ext.parameters():
        ...
        #----------------------#
        if local_rank == 0:
            eval_callback = EvalCallback(model, input_shape, class_names, num_classes, val_lines, save_dir, Cuda, eval_flag=eval_flag, period=eval_period, num_fram=num_fram)
        else:
            eval_callback = None
        #---------------------------------------#
        #   开始模型训练
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            fit_one_epoch(model_train, model, ema, yolo_loss, loss_history, eval_callback, optimizer, epoch, epoch_step, 
                        epoch_step_val, gen, gen_val, UnFreeze_Epoch, Cuda, fp16, scaler, save_period, save_dir, local_rank, file_name, num_fram)
            if distributed:
                dist.barrier()
        if local_rank == 0:
