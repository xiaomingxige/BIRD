...
def fit_one_epoch(model_train, model, ema, yolo_loss, loss_history, eval_callback, optimizer, epoch, epoch_step, 
                  epoch_step_val, gen, gen_val, Epoch, cuda, fp16, scaler, save_period, save_dir, local_rank=0, file_name='DAUB', num_fram=11):
    loss = 0
    val_loss = 0
    ...
    for iteration, batch in enumerate(gen):
        ...
        if not fp16:  # not fp16 = True
            ...
            for i in range(num_fram):
                ...
                target = targets[:, i, :, :]
                if i == 0:
                    yololoss = yolo_loss([output], target)
                else:
                    yololoss += yolo_loss([output], target)
        ...
        if local_rank == 0:
            pbar.set_postfix(**{'loss' : loss / (iteration + 1), 'yololoss' : yololoss.item() / num_fram, 'motionloss' : motionloss.item(), 
                                'lr'  : get_lr(optimizer)})
            pbar.update(1)
    if local_rank == 0:
        pbar.close()
    ...
    for iteration, batch in enumerate(gen_val):
        ...
        with torch.no_grad():
            ...
            for i in range(num_fram):
                ...
                target = targets[:, i, :, :]
                if i == 0:
                    loss_value = yolo_loss([output], target)
                else:
                    loss_value += yolo_loss([output], target)    
